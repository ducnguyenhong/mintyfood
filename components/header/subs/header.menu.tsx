...
const HeaderMenu: React.FC = () => {
  ...
  const getTextColor = useCallback((isActive: boolean) => isActive ? activeTextColor : textColor, [activeTextColor, textColor])
      ...
      {HEADER_DATA.map(item => {
        ...
        return (
          <Link href={route} key={id}>
            <a>
              <Flex alignItems="center" mx={3} bgColor={getNavBgColor(isActive)} px={6} py='6px' borderRadius={4}
                data-group
